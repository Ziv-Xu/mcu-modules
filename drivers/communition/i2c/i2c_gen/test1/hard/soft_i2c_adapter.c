...
// 发送一个字节，返回 0=成功(ACK), 1=失败(NACK/超时)
static uint8_t mpu_I2C_SendByte_CheckAck(uint8_t byte)
{
    soft_i2c_err_t ret = soft_i2c_write_byte(&i2c_bus_1, byte);
...
{
    if (mpu_I2C_SendByte_CheckAck(dev_addr << 1)) return 1;
    if (mpu_I2C_SendByte_CheckAck(reg)) return 1;
    if (mpu_I2C_SendByte_CheckAck(data)) return 1;
    mpu_I2C_Stop();
    return 0;
}
...
uint8_t mpu_I2C_Read_One_Byte(uint8_t dev_addr, uint8_t reg)
{
    uint8_t val = 0;
    mpu_I2C_Start();
    if (mpu_I2C_SendByte_CheckAck(dev_addr << 1)) goto stop;
    if (mpu_I2C_SendByte_CheckAck(reg)) goto stop;
    mpu_I2C_Start();
    if (mpu_I2C_SendByte_CheckAck((dev_addr << 1) | 0x01)) goto stop;
    soft_i2c_read_byte(&i2c_bus_1, false, &val);  // NACK
stop:
    mpu_I2C_Stop();
    return val;
}
...
{
    mpu_I2C_Start();
    if (mpu_I2C_SendByte_CheckAck(dev_addr << 1)) { mpu_I2C_Stop(); return; }
    if (mpu_I2C_SendByte_CheckAck(reg)) { mpu_I2C_Stop(); return; }
    mpu_I2C_Start();
    if (mpu_I2C_SendByte_CheckAck((dev_addr << 1) | 0x01)) { mpu_I2C_Stop(); return; }
    for (uint8_t i = 0; i < len; i++) {
        bool ack = (i < len - 1);  // 除最后一个字节外都发送 ACK
        soft_i2c_read_byte(&i2c_bus_1, ack, &buf[i]);
    }
    mpu_I2C_Stop();
}